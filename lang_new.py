...
class Vocabulary:
    ...
    def build_from_corpus(self, corpus, write_to_file=None):
        ...
        token2count = Counter(all_tokens).most_common(
            self.vocab_size)  # count corpus tokens and extract the most common ones with their counts

        # add special tokens to Vocabulary. UNK is handled separately because we have to fix its id in advance
        vocab_tokens = [self.BOS_token, self.EOS_token, self.PAD_token]
        vocab_tokens.extend([token for token, count in token2count])  # add most common corpus tokens to Vocabulary
        self.vocab_size = len(vocab_tokens) + 1  # update vocab_size (take special tokens into account). "+ 1" for UNK

        # assign an id to each vocab token, store mapping in object
        self.token2id = defaultdict(lambda: self.UNK_token_id)
        self.token2id[self.UNK_token] = self.UNK_token_id
        for token in vocab_tokens:
            # Add tokens after the already stored ones (UNK token only at the beginning)
        ...
        with open(path) as sentences:  # open corpus of sentences
            for sentence in sentences:  # for each sentence in corpus
                ...
                if (n_sentences and len(tokenized_sentences) == n_sentences):  # reached max nb of sentences to read
                    break
        self.n_sentences = len(
            tokenized_sentences)  # update self.n_sentences to nb of sentences read (in case n_sentences=None)
