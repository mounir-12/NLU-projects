...
class LSTM:
    def __init__(self, V, embedding_size, hidden_size, time_steps, clip_norm,
                 down_project=False, down_projection_size=None,
        ...
        if down_project:
            # projection matrix
            self.W_p = tf.Variable(initializer((hidden_size, down_projection_size)), name="W_p", dtype=self.dtype)
            # projection biases
            self.biases_p = tf.Variable(initializer([down_projection_size]), name="biases_p", dtype=self.dtype)
            # weights
            self.W = tf.Variable(initializer((down_projection_size, vocab_size)), name="W", dtype=self.dtype)
        else:
        ...
        if load_external_embedding:  # we're loading the embedding matrix
            matrix = load_embedding(V.token2id, embedding_path, embedding_size,
                                    vocab_size)  # load embedding matrix from file
            self.embedding_matrix = tf.Variable(matrix, name="embedding",
                                                dtype=self.dtype)  # make it trainable too (further train embeddings)
        ...
        # the input words of shape [batch_size, time_steps]
        self.input_x = tf.placeholder(tf.int32, [None, time_steps])
        # the target words of shape [batch_size, time_steps]
        self.input_y = tf.placeholder(tf.int32, [None, time_steps])
        # the embedded input of shape [batch_size, time_steps, embedding_size]
        embedded_x = tf.nn.embedding_lookup(self.embedding_matrix, self.input_x)

        # LSTM cell with hidden state of size hidden_size
        self.rnn = Cell(num_units=hidden_size, initializer=initializer, name="cell")

        # LSTM cell initial state
        self.initial_state = state = self.rnn.zero_state(tf.shape(embedded_x)[0], dtype=self.dtype)

        logits = None

        for t in range(time_steps):
            # input the slice t (i,e all embedded vectors of the batch at timestep t)
            output, state = self.rnn(embedded_x[:, t], state)
            if down_project:
                # first project then compute logit
                logit = tf.matmul(tf.matmul(output, self.W_p) + self.biases_p, self.W) + self.biases
            else:
    ...
    def get_train_op(self):  # perform gradient clipping and return train operation
        grads_and_vars = self.optimizer.compute_gradients(
            self.loss)  # compute gradient of loss function, returns list of tensor-variable pair
        ...
        return self.optimizer.apply_gradients(zip(clipped_grads, vars),
                                              self.global_step)  # apply clipped gradients and return train op
    ...
    def perplexity(self, sess, input_sentences, output_sentences, V):
        ...
        # loop over output sentences i, we consider output sentences since <EOS> is used in perplexity computation while
        # <BOS> isn't (since model shouldn't predict <BOS>)
        for i in range(s):
            # retrieve indices in the output sentence i where the token is not PAD
            indices = np.where(output_sentences[i] != pad_id)
            # compute the mean, only sum the losses vals of sentence i at the specified indices
            exponent = np.mean(losses_vals[i][indices])
    ...
    def sentence_continuation(self, sess, prompt, V, max_length):
        ...
        prompt = [V.token2id[V.BOS_token]] + prompt  # extend prompt to add BOS token
